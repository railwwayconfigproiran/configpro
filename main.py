...
def parse_config_url(config_url: str) -> dict:
    """Parse a VLESS config URL and return its query parameters.
    Handles the case where fragment (after #) contains the remark.
    """
    result = {}
    ...
    if "?" in query_string:
        query_str = query_string.split("?", 1)[1]
    else:
        query_str = ""
    for pair in query_str.split("&"):
        if "=" in pair:
            key, value = pair.split("=", 1)
            value = value.replace("+", " ").replace("%20", " ").replace("%3a", ":")
            value = value.replace("%2f", "/").replace("%25", "%")
            result[key] = value
        elif pair:
            result[pair] = ""
    if fragment:
...
def generate_vless_link_from_config(config_url: str, uid: str, remark: str = "SulgX", address: str = None, extra: dict = None) -> str:
    ...
    params.setdefault("sni", domain)
    params.setdefault("fp", "chrome")
    params.setdefault("alpn", "http/1.1")
    params.setdefault("insecure", "0")
    path = (extra.get("custom_path") or f"/ws/{uid}").format(uid=uid)
